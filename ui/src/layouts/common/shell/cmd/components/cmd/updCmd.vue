    ...
    <div style="width: 100%; height: 100%">
      <n-form ref="formRef" :label-width="80" :model="formValue" :rules="rules">
        ...
        <n-form-item label="操作系统 (不填默认兼容所有系统)" path="osTypeName">
          ...
          />
          &lt;!&ndash;          <n-space>
            <div v-for="tag in cmdOsTypeTagListRef" :key="tag" style="display: inline-block">
              <n-tag
        ...
        <n-form-item label="选项" path="options">
          <n-space>
            <div v-for="(tag, index) in formValue.cmdOptionList" :key="tag" style="display: inline-block">
              <n-tag
                style="user-select: none"
                type="info"
                closable
...
import { getSingle, upd as updCmd } from '@/theblind_shell/service/shell/cmd';
...
const osTypeFormRef = ref<any>(null);
...
/* parameter */
const parameterFormRef = ref<any>(null);
...
const confirmForm = (e: MouseEvent) => {
  ...
  formRef.value?.validate((errors: any) => {
    if (!errors) {
      updCmd(formValue.value, props.cmdId).then(resultData => {
...
function confirmOsTypeForm(e: MouseEvent) {
  ...
  osTypeFormRef.value?.validate((errors: any) => {
    if (!errors) {
...
function confirmOptionForm(e: MouseEvent) {
  ...
  optionFormRef.value?.validate((errors: any) => {
    if (!errors) {
...
function confirmParameterForm(e: MouseEvent) {
  ...
  parameterFormRef.value?.validate((errors: any) => {
    if (!errors) {
...
const initCmdData = () => {
  getSingle(props.cmdId).then(requestResult => {
    if (requestResult.data != null) {
